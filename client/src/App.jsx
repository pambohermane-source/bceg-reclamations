...
const POLES = [
  { id: "commercial", label: "Pôle Commercial", icon: "🏦",
    items: [
      { id: "cloture_compte",    label: "Clôture de compte",           extras: ["upload_lettre"] },
      { id: "changement_gest",   label: "Changement de gestionnaire",  extras: ["choix_agence"] },
      { id: "agios_trop_percu",  label: "Agios / Trop-perçu",          extras: ["montant","upload_releve"] },
      { id: "ouverture_pb",      label: "Problème ouverture de compte", extras: [] },
    ]
  },
  { id: "juridique", label: "Pôle Recouvre & Juridique", icon: "⚖️",
    items: [
      ...
      { id: "mainlevee",          label: "Mainlevée / Déblocage garantie", extras: ["upload_lettre"] },
    ]
  },
  { id: "achats", label: "Pôle Achats & Logistique", icon: "📦",
    items: [
      { id: "factures_impayees",  label: "Factures impayées",    extras: ["montant","upload_facture"] },
      { id: "bons_commande",      label: "Bons de commande",     extras: ["upload_bon"] },
      { id: "litige_fournisseur", label: "Litige fournisseur",   extras: [] },
      { id: "retard_livraison",   label: "Retard de livraison",  extras: [] },
    ]
  },
  { id: "operations", label: "Opérations Bancaires", icon: "💳",
    items: [
      { id: "virement",      label: "Problème de virement",    extras: ["montant"] },
      { id: "carte_bloquee", label: "Carte bancaire bloquée",  extras: [] },
      { id: "retrait_dab",   label: "Problème de retrait DAB", extras: ["montant"] },
      { id: "depot_pb",      label: "Problème de dépôt",       extras: ["montant"] },
    ]
  },
  { id: "service_client", label: "Service Client", icon: "⭐",
    items: [
      { id: "qualite_service",  label: "Qualité du service",           extras: [] },
      { id: "delai_traitement", label: "Délai de traitement excessif", extras: [] },
      { id: "conduite_agent",   label: "Conduite d'un agent",          extras: [] },
      { id: "demande_info",     label: "Demande d'information",        extras: [] },
    ]
  },
];
...
function Logo({ large=false }) {
  return (
    <div style={{ textAlign:"center", padding: large?"36px 24px 20px":"28px 24px 14px", position:"relative", zIndex:2 }}>
      <div style={{ fontFamily:"'Barlow Condensed','Oswald','Arial Narrow',Arial,sans-serif", fontSize:large?52:42, fontWeight:900, color:WHITE, letterSpacing:3, lineHeight:1, marginBottom:5 }}>BCEG</div>
      <div style={{ color:"rgba(255,255,255,0.5)", fontSize:10, letterSpacing:2.5 }}>Réinventons l'avenir</div>
    </div>
  );
}
...
function UZ({ hint, onFile, file }) {
  ...
  return (
    <div onClick={()=>ref.current?.click()} style={{ border:`2px dashed ${file?G2:BORDER}`, borderRadius:12, padding:"18px 14px", textAlign:"center", cursor:"pointer", background:file?"#EEF0EA":"#F8F7F4" }}>
      ...
      {file ? <><div style={{ fontSize:12, fontWeight:700, color:G2 }}>{file.name}</div><div style={{ fontSize:11, color:T_L }}>{(file.size/1024).toFixed(0)} Ko</div></> : <><div style={{ fontSize:13, fontWeight:600, color:T_D }}>Cliquez pour ajouter</div><div style={{ fontSize:11, color:T_L, marginTop:3 }}>{hint||"PDF, Image — max 10 Mo"}</div></>}
...
function LoginScreen({ onLogin, onRegister }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:BG }}>
      <GH minH={240}><Logo large /><div style={{ textAlign:"center", padding:"0 24px 32px", position:"relative", zIndex:2 }}><h1 style={{ color:WHITE, fontSize:22, fontWeight:800, marginBottom:6 }}>Connexion</h1><p style={{ color:"rgba(255,255,255,0.55)", fontSize:13 }}>Accédez à votre espace client</p></div></GH>
      <div style={{ margin:"-24px 18px 0", ...cardS, padding:"28px 22px", boxShadow:"0 12px 48px rgba(58,65,48,0.14)" }}>
        {err && <Alrt msg={err}/>}
        <div style={{ marginBottom:16 }}><label style={lbl}>Email</label><input type="email" value={f.email} placeholder="votre@email.com" onChange={e=>setF(p=>({...p,email:e.target.value}))} style={inp}/></div>
        <div style={{ marginBottom:10 }}><label style={lbl}>Mot de passe</label><div style={{ position:"relative" }}><input type={showP?"text":"password"} value={f.password} placeholder="Votre mot de passe" onChange={e=>setF(p=>({...p,password:e.target.value}))} style={{ ...inp, paddingRight:48 }} onKeyDown={e=>e.key==="Enter"&&go()}/><button onClick={()=>setShowP(p=>!p)} style={{ position:"absolute", right:14, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", cursor:"pointer", fontSize:17, color:T_M }}>{showP?"🙈":"👁️"}</button></div></div>
        ...
        <div style={{ textAlign:"center", marginTop:20, fontSize:14, color:T_M }}>Pas encore de compte ?{" "}<span onClick={onRegister} style={{ color:G2, fontWeight:700, cursor:"pointer" }}>S'inscrire</span></div>
      </div>
    ...
    <div style={{ minHeight:"100vh", background:BG }}>
      ...
      <div style={{ margin:"-20px 18px 0", ...cardS, padding:"26px 22px", boxShadow:"0 12px 48px rgba(58,65,48,0.14)" }}>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:14 }}>
          {[["prenom","Prénom *","Jean"],["nom","Nom *","DUPONT"]].map(([k,l,ph])=>(<div key={k}><label style={lbl}>{l}</label><input value={f[k]} onChange={e=>setF(p=>({...p,[k]:e.target.value}))} placeholder={ph} style={{ ...inp, borderColor:errs[k]?RED:"transparent" }}/></div>))}
        </div>
        {[["email","Email *","email","votre@email.com"],["password","Mot de passe *","password","••••••••"],["confirm","Confirmer *","password","••••••••"]].map(([k,l,t,ph])=>(<div key={k} style={{ marginBottom:14 }}><label style={lbl}>{l}</label><input type={t} value={f[k]} onChange={e=>setF(p=>({...p,[k]:e.target.value}))} placeholder={ph} style={{ ...inp, borderColor:errs[k]?RED:"transparent" }}/>{errs[k]&&typeof errs[k]==="string"&&<div style={{ color:RED, fontSize:11, marginTop:4 }}>{errs[k]}</div>}</div>))}
...
function OTPScreen({ email, onVerify, onBack }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:BG }}>
      <GH minH={240}>
        <div style={{ display:"flex", padding:"18px 18px 0", position:"relative", zIndex:2 }}><button onClick={onBack} style={{ background:"rgba(255,255,255,0.14)", border:"1px solid rgba(255,255,255,0.22)", borderRadius:10, width:36, height:36, color:WHITE, fontSize:15, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>←</button></div>
      ...
      <div style={{ margin:"-20px 18px 0", ...cardS, padding:"28px 22px", boxShadow:"0 12px 48px rgba(58,65,48,0.14)" }}>
        <Alrt msg="Code démo : 123456" type="info"/>
        ...
        <div style={{ display:"flex", gap:8, justifyContent:"center", marginBottom:24 }}>
          {otp.map((d,i)=>(<input key={i} ref={el=>refs.current[i]=el} value={d} maxLength={1} inputMode="numeric" onChange={e=>onD(i,e.target.value)} onKeyDown={e=>onK(i,e)} onPaste={onP} style={{ width:42, height:50, textAlign:"center", fontSize:22, fontWeight:800, background:d?G2:"#EEECEA", color:d?WHITE:T_D, border:`2px solid ${d?G2:BORDER}`, borderRadius:10, outline:"none", cursor:"text", transition:"all .18s", fontFamily:"inherit" }}/>))}
        </div>
        {err && <Alrt msg={err}/>}
        ...
        <div style={{ textAlign:"center", marginTop:16, fontSize:14, color:T_M }}>{timer>0?<span>Renvoyer le code ({timer}s)</span>:<span style={{ color:G2, fontWeight:700, cursor:"pointer" }} onClick={()=>setTimer(60)}>Renvoyer le code</span>}</div>
      </div>
    </div>
  );
}
      ...
      <GH minH={0}>
        ...
        <div style={{ margin:"14px 18px 0", background:"rgba(0,0,0,0.15)", borderRadius:14, padding:"14px 0", border:"1px solid rgba(255,255,255,0.1)" }}>
          <div style={{ display:"flex" }}>
            {[["0","Conversations"],[String(claims.length),"Réclamations"],[String(enCours),"En cours"]].map(([n,l],i)=>(<div key={i} style={{ flex:1, textAlign:"center", borderRight:i<2?"1px solid rgba(255,255,255,0.15)":"none" }}><div style={{ color:WHITE, fontSize:22, fontWeight:800 }}>{n}</div><div style={{ color:"rgba(255,255,255,0.5)", fontSize:11 }}>{l}</div></div>))}
          </div>
        </div>
        <div style={{ height:28 }}/>
      </GH>
      <div style={{ padding:"16px 16px 0" }}>
        ...
        {claims.length===0 ? (
          ...
          <div>
            ...
            {claims.slice(0,4).map(c=>{ const sc={Nouvelle:["#DBEAFE","#1D4ED8"],"En traitement":["#FEF9C3","#7B5800"],Clôturée:["#DCFCE7","#166534"],Rejetée:["#FEE2E2","#991B1B"]}[c.statut]||["#F3F4F6","#374151"]; const cat=POLES.find(p=>p.id===c.pole)?.items.find(i=>i.id===c.category)?.label||c.category; return (<div key={c.id} style={{ ...cardS, display:"flex", alignItems:"center", gap:12, padding:"13px 15px", marginBottom:8 }}><div style={{ width:38, height:38, borderRadius:12, background:"#EEF0EA", display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, flexShrink:0 }}>📋</div><div style={{ flex:1, minWidth:0 }}><div style={{ fontSize:13, fontWeight:700, color:T_D, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{cat}</div><div style={{ fontSize:11, color:T_L }}>{c.id} · {c.date}</div></div><div style={{ padding:"4px 10px", borderRadius:20, background:sc[0], color:sc[1], fontSize:10, fontWeight:700, whiteSpace:"nowrap" }}>{c.statut}</div></div>); })}
          </div>
        )}
      </div>
    ...
    <div style={{ minHeight:"100vh", background:BG }}>
      <GH minH={0}>
        ...
        <div style={{ padding:"14px 18px 22px", display:"flex", gap:0, alignItems:"center", position:"relative", zIndex:2 }}>
          {SL.map((l,i)=>(<div key={i} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", position:"relative" }}>{i>0&&<div style={{ position:"absolute", top:13, right:"50%", width:"100%", height:2, background:i<=step-1?"rgba(255,255,255,0.65)":"rgba(255,255,255,0.2)" }}/>}<div style={{ width:26, height:26, borderRadius:"50%", position:"relative", zIndex:1, background:i<step-1?"rgba(255,255,255,0.85)":i===step-1?WHITE:"rgba(255,255,255,0.2)", display:"flex", alignItems:"center", justifyContent:"center", marginBottom:4, boxShadow:i===step-1?"0 0 0 4px rgba(255,255,255,0.2)":"none" }}><span style={{ fontSize:11, fontWeight:800, color:i<=step-1?G2:"rgba(255,255,255,0.5)" }}>{i<step-1?"✓":i+1}</span></div><span style={{ fontSize:9, color:i===step-1?WHITE:"rgba(255,255,255,0.45)", fontWeight:i===step-1?700:400, textAlign:"center" }}>{l}</span></div>))}
        </div>
      </GH>
      <div style={{ padding:"16px 16px 100px" }}>
        {step===1&&(<div style={cardS}><h3 style={{ fontSize:15, fontWeight:800, color:T_D, marginBottom:4 }}>Quel est votre profil ?</h3><p style={{ fontSize:12, color:T_M, marginBottom:18 }}>Sélectionnez la catégorie qui vous correspond</p><div style={{ display:"flex", flexDirection:"column", gap:10 }}>{PROFILES.map(p=>(<button key={p.id} onClick={()=>setForm(f=>({...f,profile:p.id}))} style={{ display:"flex", alignItems:"center", gap:14, padding:"13px 15px", borderRadius:12, border:`2px solid ${form.profile===p.id?G2:BORDER}`, background:form.profile===p.id?"#EEF0EA":WHITE, cursor:"pointer", textAlign:"left", fontFamily:"inherit" }}><div style={{ width:44, height:44, borderRadius:12, background:form.profile===p.id?G2:BG, display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, flexShrink:0 }}>{p.icon}</div><div style={{ flex:1 }}><div style={{ fontSize:14, fontWeight:700, color:form.profile===p.id?G2:T_D }}>{p.label}</div><div style={{ fontSize:12, color:T_L }}>{p.desc}</div></div>{form.profile===p.id&&<div style={{ color:G2, fontSize:18 }}>✓</div>}</button>))}</div></div>)}
        {step===2&&(<div><div style={cardS}><h3 style={{ fontSize:15, fontWeight:800, color:T_D, marginBottom:4 }}>Choisissez un pôle</h3><p style={{ fontSize:12, color:T_M, marginBottom:16 }}>Quel service est concerné ?</p><div style={{ display:"flex", flexDirection:"column", gap:9 }}>{POLES.map(p=>(<button key={p.id} onClick={()=>setForm(f=>({...f,pole:p.id,category:""}))} style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 14px", borderRadius:12, border:`2px solid ${form.pole===p.id?G2:BORDER}`, background:form.pole===p.id?"#EEF0EA":WHITE, cursor:"pointer", textAlign:"left", fontFamily:"inherit" }}><div style={{ width:40, height:40, borderRadius:12, background:form.pole===p.id?G2:BG, display:"flex", alignItems:"center", justifyContent:"center", fontSize:18 }}>{p.icon}</div><span style={{ fontSize:13, fontWeight:700, color:form.pole===p.id?G2:T_D, flex:1 }}>{p.label}</span>{form.pole===p.id&&<span style={{ color:G2, fontSize:16 }}>✓</span>}</button>))}</div></div>{form.pole&&(<div style={cardS}><h3 style={{ fontSize:14, fontWeight:800, color:T_D, marginBottom:14 }}>Type de réclamation</h3><div style={{ display:"flex", flexDirection:"column", gap:8 }}>{POLES.find(p=>p.id===form.pole)?.items.map(item=>(<button key={item.id} onClick={()=>setForm(f=>({...f,category:item.id}))} style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 14px", borderRadius:10, border:`2px solid ${form.category===item.id?G2:BORDER}`, background:form.category===item.id?"#EEF0EA":"#FAFAF8", cursor:"pointer", textAlign:"left", fontFamily:"inherit" }}><div style={{ width:8, height:8, borderRadius:"50%", background:form.category===item.id?G2:GRAY, flexShrink:0 }}/><span style={{ fontSize:14, fontWeight:600, color:form.category===item.id?G2:T_D, flex:1 }}>{item.label}</span>{item.extras.length>0&&<span style={{ fontSize:10, color:T_L, background:BG, padding:"2px 8px", borderRadius:20 }}>+docs</span>}{form.category===item.id&&<span style={{ color:G2, fontSize:15 }}>✓</span>}</button>))}</div></div>)}</div>)}
        {step===3&&(<div style={cardS}><h3 style={{ fontSize:15, fontWeight:800, color:T_D, marginBottom:4 }}>Détails de votre réclamation</h3><p style={{ fontSize:12, color:T_M, marginBottom:18 }}>Plus vous êtes précis, plus vite nous traitons votre dossier.</p><div style={{ marginBottom:16 }}><label style={lbl}>Agence concernée</label><select value={form.agence} onChange={e=>setForm(f=>({...f,agence:e.target.value}))} style={inp}><option value="">-- Sélectionnez --</option>{AGENCES.map(a=><option key={a}>{a}</option>)}</select></div>{extras.includes("montant")&&<div style={{ marginBottom:16 }}><label style={lbl}>Montant concerné (FCFA)</label><input type="number" value={form.montant} onChange={e=>setForm(f=>({...f,montant:e.target.value}))} placeholder="Ex : 150 000" style={inp}/></div>}{extras.includes("choix_agence")&&<div style={{ marginBottom:16 }}><label style={lbl}>Agence pour le nouveau gestionnaire</label><select value={form.agenceNouvelle} onChange={e=>setForm(f=>({...f,agenceNouvelle:e.target.value}))} style={inp}><option value="">-- Sélectionnez --</option>{AGENCES.map(a=><option key={a}>{a}</option>)}</select></div>}<div style={{ marginBottom:14 }}><label style={lbl}>Description détaillée *</label><textarea value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))} placeholder="Décrivez précisément : date des faits, montant, numéro d'opération..." rows={5} style={{ ...inp, resize:"vertical", lineHeight:1.6, fontFamily:"inherit" }}/></div>{extras.filter(e=>e.startsWith("upload")).map(ext=>{ const UL={upload_lettre:"Joindre la lettre",upload_releve:"Joindre le relevé",upload_facture:"Joindre la facture",upload_bon:"Joindre le bon"}; return <div key={ext} style={{ marginBottom:14 }}><label style={{ ...lbl, marginBottom:8 }}>{UL[ext]||"Document annexe"}</label><UZ hint="PDF, Image" onFile={file=>setForm(f=>({...f,files:{...f.files,[ext]:file}}))} file={form.files[ext]}/></div>; })}</div>)}
        {step===4&&(<div><div style={{ ...cardS, background:"#F5F0E4", border:"1px solid #E8DC8A", display:"flex", gap:12, padding:"16px" }}><span style={{ fontSize:22 }}>🔒</span><div><div style={{ fontSize:13, fontWeight:700, color:"#6B5A00", marginBottom:3 }}>Vérification d'identité requise</div><div style={{ fontSize:12, color:"#7B6A10", lineHeight:1.5 }}>Une pièce d'identité valide et votre numéro de téléphone sont obligatoires.</div></div></div><div style={cardS}><h3 style={{ fontSize:14, fontWeight:800, color:T_D, marginBottom:14 }}>Pièce d'identité *</h3><UZ hint="CNI, Passeport — PDF, Image" onFile={file=>setForm(f=>({...f,files:{...f.files,identite:file}}))} file={form.files.identite}/></div><div style={cardS}><h3 style={{ fontSize:14, fontWeight:800, color:T_D, marginBottom:14 }}>Coordonnées de contact</h3><div style={{ marginBottom:14 }}><label style={lbl}>Numéro de téléphone *</label><input type="tel" value={form.telephone} onChange={e=>setForm(f=>({...f,telephone:e.target.value}))} placeholder="06 12 34 56" style={inp}/></div><div><label style={lbl}>Email de suivi (optionnel)</label><input type="email" value={form.emailSuivi} onChange={e=>setForm(f=>({...f,emailSuivi:e.target.value}))} placeholder="votre@email.com" style={inp}/></div></div><div style={cardS}><div style={{ fontSize:10, fontWeight:700, color:T_L, letterSpacing:1.2, marginBottom:12 }}>RÉCAPITULATIF</div>{[["Profil",PROFILES.find(p=>p.id===form.profile)?.label],["Pôle",pole?.label],["Réclamation",item?.label],["Agence",form.agence||"Non précisée"],form.montant?["Montant",form.montant+" FCFA"]:null].filter(Boolean).map(([k,v])=>(<div key={k} style={{ display:"flex", justifyContent:"space-between", padding:"9px 0", borderBottom:"1px solid "+BORDER }}><span style={{ fontSize:12, color:T_M }}>{k}</span><span style={{ fontSize:12, fontWeight:700, color:T_D, maxWidth:"60%", textAlign:"right" }}>{v||"—"}</span></div>))}</div></div>)}
      </div>
      <div style={{ position:"fixed", bottom:0, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:430, padding:"14px 18px 20px", background:"rgba(245,243,239,0.96)", backdropFilter:"blur(12px)", borderTop:"1px solid "+BORDER, boxSizing:"border-box" }}>
        {step<4?<button onClick={()=>setStep(s=>s+1)} disabled={!can[step-1]} style={gbtn({ opacity:can[step-1]?1:.4, cursor:can[step-1]?"pointer":"default" })}>Suivant →</button>:<button onClick={()=>onSubmit(form)} disabled={!can[3]} style={gbtn({ opacity:can[3]?1:.4, cursor:can[3]?"pointer":"default" })}>✓ Soumettre la réclamation</button>}
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ minHeight:"100vh", background:BG }}>
      ...
      <div style={{ padding:"16px 16px 40px" }}>
        {claims.length===0?(<div style={{ ...cardS, textAlign:"center", padding:"48px 24px" }}><div style={{ fontSize:40, marginBottom:12 }}>📂</div><h3 style={{ fontSize:15, fontWeight:700, color:T_D, marginBottom:8 }}>Aucun dossier pour le moment</h3><p style={{ fontSize:13, color:T_L, marginBottom:20 }}>Déposez votre première réclamation</p><button onClick={onNewClaim} style={{ ...gbtn(), width:"auto", padding:"12px 28px" }}>Déposer une réclamation</button></div>):(claims.map(c=>{ const sc=SC[c.statut]||["#F3F4F6","#374151"]; const polN=POLES.find(p=>p.id===c.pole)?.label||c.pole; const catN=POLES.find(p=>p.id===c.pole)?.items.find(i=>i.id===c.category)?.label||c.category; return (<div key={c.id} style={{ ...cardS, marginBottom:10, position:"relative", overflow:"hidden" }}><Bark w={18} h={70} op={0.07} col={G2}/><div style={{ paddingLeft:8 }}><div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:10 }}><div style={{ flex:1, marginRight:10 }}><div style={{ fontSize:14, fontWeight:800, color:T_D, marginBottom:3 }}>{catN}</div><div style={{ fontSize:11, color:T_L }}>{polN}</div></div><div style={{ padding:"4px 10px", borderRadius:20, background:sc[0], color:sc[1], fontSize:10, fontWeight:700, whiteSpace:"nowrap" }}>{c.statut}</div></div><div style={{ display:"flex", gap:16, paddingTop:10, borderTop:"1px solid "+BORDER, flexWrap:"wrap" }}>{[["Référence",c.id,true],["Date",c.date,false],["Profil",PROFILES.find(p=>p.id===c.profile)?.label||"—",false]].map(([k,v,m])=>(<div key={k}><div style={{ fontSize:10, color:T_L, marginBottom:2 }}>{k}</div><div style={{ fontSize:12, fontWeight:700, color:T_D, fontFamily:m?"'Barlow Condensed',monospace":"inherit" }}>{v}</div></div>))}</div></div></div>); }))}
      </div>
    </div>
  );
}
...
function ProfileScreen({ user, onBack }) {
  return (
    <div style={{ minHeight:"100vh", background:BG }}>
      <GH minH={0}><TopBar onBack={onBack} title="Mon Profil"/><div style={{ textAlign:"center", padding:"18px 24px 24px", position:"relative", zIndex:2 }}><div style={{ display:"inline-flex", width:72, height:72, borderRadius:"50%", background:"rgba(255,255,255,0.18)", border:"2.5px solid rgba(255,255,255,0.36)", alignItems:"center", justifyContent:"center", fontSize:28, fontWeight:800, color:WHITE, marginBottom:10 }}>{user?.prenom?.[0]}</div><div style={{ color:WHITE, fontSize:17, fontWeight:800 }}>{user?.prenom} {user?.nom}</div><div style={{ color:"rgba(255,255,255,0.45)", fontSize:11, marginTop:3 }}>Client BCEG</div></div></GH>
      <div style={{ padding:"16px 16px 40px" }}>
        {[["Prénom",user?.prenom],["Nom",user?.nom],["Email",user?.email],["Téléphone","Non renseigné"],["Agence principale","Non renseignée"],["Type de client","Particulier"]].map(([k,v])=>(<div key={k} style={{ ...cardS, display:"flex", justifyContent:"space-between", alignItems:"center", padding:"15px 18px", marginBottom:8 }}><span style={{ fontSize:13, color:T_M }}>{k}</span><span style={{ fontSize:13, fontWeight:700, color:T_D }}>{v}</span></div>))}
